...
import { fetchSmsAccount, smsQueryOptions } from "../../lib/modirpayamak-api"
...
export default function PageClient({ route: _route }: { route: ResolvedAdminRoute }) {
  ...
  return (
    <div className="space-y-4 p-4 md:p-6">
      <div>
        <h1 className="text-xl font-semibold">{t("title")} — {t("pages.wallet")}</h1>
        <p className="text-sm text-muted-foreground">{t("description")}</p>
      </div>
      <SmsNav />
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t("pages.wallet")}</CardTitle>
        </CardHeader>
        <CardContent>
