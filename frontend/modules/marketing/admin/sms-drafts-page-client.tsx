...
import { fetchSmsDrafts, smsQueryOptions } from "../../lib/modirpayamak-api"
...
export default function PageClient({ route: _route }: { route: ResolvedAdminRoute }) {
  ...
  const q = useQuery({
    queryKey: ["sms", "sms-drafts"],
    queryFn: () => fetchSmsDrafts(),
    ...smsQueryOptions,
  })
  ...
  return (
    <div className="space-y-4 p-4 md:p-6">
      <div>
        <h1 className="text-xl font-semibold">{t("title")} — {t("pages.drafts")}</h1>
        <p className="text-sm text-muted-foreground">{t("description")}</p>
      </div>
      <SmsNav />
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t("pages.drafts")}</CardTitle>
        </CardHeader>
        <CardContent>
